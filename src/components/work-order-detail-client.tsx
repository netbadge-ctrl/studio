...
import { Lightbulb, LightbulbOff, QrCode, MapPin, Layers, Server as ServerIcon, HardDrive, MemoryStick, Cpu, ArrowRight, Network } from 'lucide-react';
...
function DeviceDetail({ device }: { device: Device }) {
  ...
  }).filter(item => item.type); // Filter out items with no type (for empty configs)
          ...
          <CardHeader>
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                <div>
                    ...
                    <CardDescription className="font-mono font-code">{device.serialNumber}</CardDescription>
                </div>
            </div>
          </CardHeader>
          <CardContent>
...
export function WorkOrderDetailClient({ workOrder }: { workOrder: WorkOrder }) {
  if (workOrder.devices.length === 1) {
    return (
      <>
        ...
        <DeviceDetail device={workOrder.devices[0]} />
      </>
    );
  }
  ...
  return (
    <Card>
        ...
        <CardContent>
            <Tabs defaultValue={workOrder.devices[0].id}>
                <TabsList>
                    {workOrder.devices.map(device => (
                        <TabsTrigger key={device.id} value={device.id} className="flex items-center">
                           {getDeviceIcon(device.type)} {device.type} ({device.serialNumber.slice(-6)})
                        </TabsTrigger>
                    ))}
                </TabsList>
                {workOrder.devices.map(device => (
